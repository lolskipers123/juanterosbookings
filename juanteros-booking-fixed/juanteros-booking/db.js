...
async function init() {
  ...
  // outside /tmp, and /tmp doesn't persist between requests anyway - so the
  // local JSON-file mode below silently loses every booking. Fail loudly
  // and early instead, with a message that points at the actual fix.
  if (!usePostgres && process.env.VERCEL) {
    throw new Error(
      'DATABASE_URL is not set. On Vercel this app needs a real database ' +
      '(Postgres/Supabase) since the filesystem is read-only and nothing ' +
      'written to it survives between requests. Add DATABASE_URL in your ' +
      'Vercel project\'s Settings -> Environment Variables, using the ' +
      'connection string from Supabase (Project Settings -> Database -> ' +
      'Connection string -> URI), then redeploy.'
    );
  }
