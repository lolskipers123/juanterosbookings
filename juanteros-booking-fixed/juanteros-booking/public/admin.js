// Cache-busts a normal image URL so the browser doesn't show a stale
// cached photo after a re-upload. Uploaded photos are now stored as
// data: URIs (base64), and appending "?v=..." text to the end of one of
// those corrupts it - it's already a fresh value on every upload, so it
// never needs cache-busting in the first place.
function imgSrc(url) {
  if (!url) return '';
  return url.startsWith('data:') ? url : `${url}?v=${Date.now()}`;
}
      ...
      rooms.forEach(room => {
        ...
        card.innerHTML = `
          <div class="photo-preview">
            <img src="${imgSrc(room.image)}" alt="${escapeHTML(room.name)}"
  ...
  fetch('/api/site-images')
    ...
    .then(siteImages => {
      Object.keys(siteImages).forEach(key => {
        ...
        card.innerHTML = `
          <div class="photo-preview">
            <img src="${imgSrc(entry.image)}" alt="${escapeHTML(entry.label)}"
                 onerror="this.style.display='none';">
          </div>
          <div class="photo-body">
  ...
  fetch('/api/payment-settings')
    ...
    .then(paymentSettings => {
      Object.keys(paymentSettings).forEach(key => {
        ...
        card.innerHTML = `
          <div class="photo-preview">
            <img src="${imgSrc(m.qrImage)}" alt="${escapeHTML(m.label)} QR code"
                 onerror="this.style.display='none';">
          </div>
          <div class="photo-body">
