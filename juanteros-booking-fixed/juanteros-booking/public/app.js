  ...
  fetch('/api/site-images')
    ...
    .then(siteImages => {
      Object.keys(siteImages).forEach(key => {
        ...
        if (!src) return;
        // Uploaded photos are stored as data: URIs (base64) - appending
        // "?v=..." to one of those corrupts it, and it's already unique
        // per upload so it never needs cache-busting anyway.
        const bustedSrc = src.startsWith('data:') ? src : `${src}?v=${Date.now()}`;
