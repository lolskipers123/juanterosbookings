...
const PAYMENT_PROOFS_DIR = path.join(__dirname, 'public', 'uploads', 'payment-proofs');
...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      // Timestamped filename so browsers never serve a stale cached photo
...
app.post('/api/rooms/:id/image', requireAdminApi, (req, res) => {
  upload.single('photo')(req, res, async (err) => {
    ...
    if (!room) {
      // Clean up the file we just saved since it has nowhere to go
      fs.unlink(req.file.path, () => {});
      return res.status(404).json({ error: 'Room not found.' });
    }

    // Remove the old photo file if it lives in our images folder
    // (skip if it's missing or was never a local upload)
    const oldImagePath = room.image ? path.join(__dirname, 'public', room.image) : null;
    if (oldImagePath && oldImagePath.startsWith(IMAGES_DIR) && fs.existsSync(oldImagePath)) {
      fs.unlink(oldImagePath, () => {});
    }

    room.image = `images/${req.file.filename}`;
...
const uploadPaymentProof = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => cb(null, PAYMENT_PROOFS_DIR),
    filename: (req, file, cb) => {
      const ext = path.extname(file.originalname).toLowerCase();
      cb(null, `${req.params.id}-${Date.now()}${ext}`);
    }
...
app.post('/api/site-images/:key/image', requireAdminApi, (req, res) => {
  uploadSiteImage.single('photo')(req, res, async (err) => {
    ...
    // Remove the old photo file if it lives in our images folder
    const oldImagePath = siteImages[key].image ? path.join(__dirname, 'public', siteImages[key].image) : null;
    if (oldImagePath && oldImagePath.startsWith(IMAGES_DIR) && fs.existsSync(oldImagePath)) {
      fs.unlink(oldImagePath, () => {});
    }

    siteImages[key].image = `images/${req.file.filename}`;
...
app.post('/api/payment-settings/:key/image', requireAdminApi, (req, res) => {
  uploadSiteImage.single('photo')(req, res, async (err) => {
    ...
    const oldImagePath = paymentSettings[key].qrImage ? path.join(__dirname, 'public', paymentSettings[key].qrImage) : null;
    if (oldImagePath && oldImagePath.startsWith(IMAGES_DIR) && fs.existsSync(oldImagePath)) {
      fs.unlink(oldImagePath, () => {});
    }

    paymentSettings[key].qrImage = `images/${req.file.filename}`;
    await db.savePaymentSettings(paymentSettings);
...
app.post('/api/bookings/:id/payment-proof', (req, res) => {
  uploadPaymentProof.single('proofImage')(req, res, async (err) => {
    ...
    // Replace any previously uploaded proof file for this booking
    const oldProofPath = bookings[index].paymentProofImage
      ? path.join(__dirname, 'public', bookings[index].paymentProofImage)
      : null;
    if (oldProofPath && oldProofPath.startsWith(PAYMENT_PROOFS_DIR) && fs.existsSync(oldProofPath)) {
      fs.unlink(oldProofPath, () => {});
    }

    bookings[index].paymentMethod = method;
    bookings[index].paymentReference = reference || '';
    bookings[index].paymentProofImage = `uploads/payment-proofs/${req.file.filename}`;
